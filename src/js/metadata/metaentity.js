...
import operationManager from "../../components/meta_operation/js/common_operation";
export default function (options) {
    ...
    metaEntity.dataResource = function () {
        ...
        _.forIn(this.relations, (metaRelation, key) => {
            ...
            var pathName = name;
            if (type == 'one-to-many') {
                relationActions[name] = {method: 'GET', url: `${this.entityPath}{/parentEntityId}/${pathName}`}
            }
        });
        //var dataResource = context.buildResource(resourceName, customActions, {root: this.engineUrl});
        var dataResource = restResource(resourceName, relationActions, {root: this.engineUrl});
        return dataResource;
    }
    ...
    metaEntity.getPage = function (pageKey) {
        return new Promise((resolve,reject)=>{
          this.getUISettings().then(ui=>{
            var page = null;
            if (ui.pages != null) {
                page = ui.pages[pageKey];
            }
            resolve(_.cloneDeep(page));
          })
        });
    }

    metaEntity.getRelationUI = function (relationName) {
        ...
        var resource = this.dataResource();
        return new Promise((resolve,reject)=>{
          resource.relationUI({relation:relationName}).then(resp=>{
            ui=resp.data;
    ...
    metaEntity.getRelationPage = function (relationName,pageKey) {
      return new Promise((resolve,reject)=>{
        this.getRelationUI(relationName).then(relationUI=>{
          if(relationUI){
              var page= null;
              if(relationUI.pages){
